...
import { createClass, clearMessage } from './actions';
import {
  makeSelectMessage,
  makeSelectLoading,
} from './selectors';
...
export class ClassCreate extends React.PureComponent {
  ...
  onSubmit() {
    ...
    if (!Object.keys(error).length) {
      this.props.createClass(this.state.formData);
    } else {
      this.setState({ error });
    }
  }
        ...
        <Container fluid>
          <Row>
            <Col xl={6}>
              <Card>
                <CardHeader>
                  <i className="fa fa-align-justify"></i> Tạo mới lớp
                </CardHeader>
                <CardBody>
...
const withReducer = injectReducer({ key: 'classCreate', reducer });
const withSaga = injectSaga({ key: 'classCreate', saga });
